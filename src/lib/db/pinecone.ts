...
export async function loadS3IntoPinecone(filekey: string) {
  ...
  for (let i = 0; i < vectorChunks.length; i++) {
    ...
    const validRecords = chunk.filter(record => record.values && record.values.length > 0);
...
export const truncateStringByBytes = (str: string, numBytes: number): string => {
...
async function checkNamespaceExists(index: any, namespace: string | null): Promise<boolean> {
